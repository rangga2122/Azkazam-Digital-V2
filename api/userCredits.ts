...
function toCamel(row: any) {
  ...
  return {
    ...
    // Penting: kembalikan allowedFeatures agar UI tidak menganggap semua fitur terbuka
    allowedFeatures: Array.isArray(row.allowed_features) ? row.allowed_features : [],
  };
}
  ...
  try {
    ...
    const ensureUserRow = async (): Promise<any | null> => {
      ...
      const row = {
        ...
        expiry_date: null,
        // Default: tidak ada fitur yang diizinkan sampai admin menetapkan
        allowed_features: []
      };
