...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'POST') {
      const { email, password, role = 'user', dailyLimit = 5, expiryDate, allowedFeatures, maxSessions } = req.body || {};
      ...
      const row = {
        ...
        allowed_features: Array.isArray(allowedFeatures) ? allowedFeatures : null,
    ...
    if (req.method === 'PUT') {
      ...
      if (body.allowedFeatures !== undefined) patch.allowed_features = Array.isArray(body.allowedFeatures) ? body.allowedFeatures : null;
      if (body.maxSessions !== undefined) patch.max_sessions = Math.max(Number(body.maxSessions || 1) || 1, 1);

      // Do nothing if patch empty
