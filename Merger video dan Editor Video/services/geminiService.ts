...
async function getGeminiModel(kind: 'text' | 'tts'): Promise<string> {
    try {
        ...
        if (kind === 'text') cachedTextModel = val;
        else cachedTtsModel = val;
        cachedModelAt = Date.now();
...
Hashtags: [#tag1 #tag2 #tag3 #tag4 #tag5]`;
        ...
        throw new Error('Kuota/limit API Gemini tercapai atau jaringan bermasalah. Ganti API key baru lalu coba lagi.');
    } catch (error) {
...
// Vision: Generate short lip-sync friendly script from product image, optionally
// grounded with a model/reference image and theme description.
export const generateLipsyncScriptFromImage = async (
    productImageFile: File,
    style: string,
    themeDescription: string | undefined,
    modelImageFile: File | undefined,
    onProgress: (message: string) => void,
    language: string = 'id'
): Promise<string> => {
    try {
        if (!productImageFile) throw new Error('Silakan unggah foto produk terlebih dahulu.');
        onProgress('Mengunggah foto produk...');

        const base64Product = arrayBufferToBase64(await productImageFile.arrayBuffer());
        const base64Model = modelImageFile ? arrayBufferToBase64(await modelImageFile.arrayBuffer()) : undefined;
        ...
        const languageText2 = langLabelMap2[language] || 'bahasa indonesia';
        const styleText = style ? `Gaya narasi: ${style}.` : 'Gaya narasi: Santai.';
        const rawTheme = (themeDescription || '').trim();
        const isSequence4 = /\[SEQUENCE_4\]/.test(rawTheme);
        const partMatch = rawTheme.match(/\[PART=(\d)\]/);
        const partIndex = partMatch ? Math.max(1, Math.min(4, parseInt(partMatch[1], 10))) : 0;
        const themeClean = rawTheme
            .replace(/\[SEQUENCE_4\]/g, '')
            .replace(/\[PART=\d\]/g, '')
            .trim();
        const themeText = themeClean ? (/deskripsi\s+produk/i.test(themeClean) ? themeClean : `Tema visual: ${themeClean}.`) : '';

        // Prompt diarahkan untuk skrip voiceover sangat pendek agar pas 8 detik total (4 bagian)
        let prompt: string;
        if (isSequence4 && partIndex >= 1 && partIndex <= 4) {
            const partInstructions = [
                'Bagian 1 (Hook): satu kalimat 5–10 kata, tarik perhatian, relevan dengan visual & produk.',
                'Bagian 2 (Masalah/Need): satu kalimat 5–10 kata, ungkap kebutuhan/masalah ringan yang relatable.',
                'Bagian 3 (Solusi/Manfaat): satu kalimat 5–10 kata, nyatakan manfaat utama/hasil yang dirasakan.',
                'Bagian 4 (CTA): satu kalimat 3–6 kata, ajakan kuat & jelas (mis. Coba sekarang).'
            ];
            const brevity = 'Sangat pendek, tepat satu kalimat, cocok untuk ~2 detik durasi.';
            prompt = `Anda adalah copywriter iklan berpengalaman. Dari gambar terlampir, buat skrip lipsync super singkat dalam ${languageText2}. ${styleText} ${themeText}

Instruksi wajib:
- ${guidance}
- Grounding ke gambar: sebutkan atribut yang terlihat (jenis produk, bahan/tekstur, warna/pola, suasana/aktivitas) secara natural.
- ${brevity}
- Fokus: ${partInstructions[partIndex - 1]}
- Hindari kata "gambar" atau "produk ini"; deskripsikan natural sesuai visual.
- Tanpa bullet/emoji/markdown.`;
        } else {
            // Default mode (non sequence)
            prompt = `Anda adalah copywriter iklan berpengalaman. Dari gambar terlampir, buat skrip voiceover lip-sync singkat dalam ${languageText2}. ${styleText} ${themeText}

Instruksi wajib:
- ${guidance}
- Grounding ke gambar: sebutkan atribut yang terlihat (jenis produk, bahan/tekstur, warna/pola, suasana/aktivitas) secara natural.
- Kalimat pendek 6–12 kata agar mudah di-lipsync. Maksimal 3–4 kalimat, total 50–90 kata.
- Hindari kata "gambar" atau "produk ini"; deskripsikan natural sesuai visual.
- Akhiri kalimat terakhir dengan CTA ringan (mis. Coba sekarang, Dapatkan sekarang).
- Tanpa bullet/emoji/markdown.`;
        }

        onProgress('Meminta Chutes AI...');
        ...
        try {
            ...
            scriptText = String(obj?.script || '').trim();
        } catch {
            scriptText = cleaned;
        }
        if (!scriptText) throw new Error('Skrip kosong');
        onProgress('🎉 Skrip lipsync berhasil dibuat!');
        return scriptText;
    } catch (error) {
        console.error('Error generate lipsync script from image:', error);
        if (error instanceof Error) throw error;
        throw new Error('Gagal membuat skrip lipsync dari gambar');
    }
};
